...
# 
# Examples:
# - To create new session: Add line "99:sessionName" 
# - To rename session 1: Change "1:oldname" to "1:newname"
# - To kill session: Delete the line (WARNING: this will kill the session!)
#
EOF
}
...
mark_current_session() {
    if [ -n "$TMUX" ]; then
        ...
        sed -i "s/:${current_session}$/:${current_session} */" "$TEMP_FILE" 2>/dev/null || true
    fi
}
