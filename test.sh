...
load_options() {
    ...
    # New options for cache expiry
    TMUXIFIER_CACHE_EXPIRY=$(get_tmux_option "@omnimux-tmuxifier-cache-expiry" "5") # seconds
    ZOXIDE_CACHE_EXPIRY=$(get_tmux_option "@omnimux-zoxide-cache-expiry" "10")     # seconds
    FIND_CACHE_EXPIRY=$(get_tmux_option "@omnimux-find-cache-expiry" "300")       # seconds (5 minutes)
    ACTIVE_SESSIONS_CACHE_EXPIRY=$(get_tmux_option "@omnimux-active-sessions-cache-expiry" "2") # seconds
}

find_tmuxifier() {
    local cache_file="$CACHE_DIR/tmuxifier_dir"
    
    # We always read from cache if available and then update in background
...
get_tmux_sessions() {
    ...
    if [ -f "$cache_file" ] && [ $((current_time - $(stat -c %Y "$cache_file" 2>/dev/null || echo 0))) -lt "$ACTIVE_SESSIONS_CACHE_EXPIRY" ]; then
        cat "$cache_file"
        return 0
    fi
...
"
    ...
    if [ -n "$current_session_line" ]; then
        sessions_output="$current_session_line"
        if [ -n "$other_sessions" ]; then
            sessions_output="${sessions_output}
${other_sessions}"
        fi
    else
        sessions_output="$other_sessions"
    fi
    
    echo "$sessions_output" | sed '/^$/d' | tee "$cache_file"
}
...
get_tmuxifier_sessions() {
    ...
    if [ -f "$cache_file" ] && [ $((current_time - $(stat -c %Y "$cache_file" 2>/dev/null || echo 0))) -lt "$TMUXIFIER_CACHE_EXPIRY" ]; then
        cat "$cache_file"
        return 0
    fi
...
get_zoxide_paths() {
    ...
    if [ -f "$cache_file" ] && [ $((current_time - $(stat -c %Y "$cache_file" 2>/dev/null || echo 0))) -lt "$ZOXIDE_CACHE_EXPIRY" ]; then
        cat "$cache_file"
        return 0
    fi
...
get_find_paths() {
    ...
    if [ -f "$cache_file" ] && [ $((current_time - $(stat -c %Y "$cache_file" 2>/dev/null || echo 0))) -lt "$FIND_CACHE_EXPIRY" ]; then
        cat "$cache_file"
        return 0
    fi
...
filter_tmuxifier_sessions() {
    local cache_file="$CACHE_DIR/active_sessions" # This cache is used by get_tmux_sessions now.
    
    # We rely on get_tmux_sessions to update the active sessions cache
    local active_sessions=$(cat "$cache_file" 2>/dev/null || get_tmux_sessions | tee "$cache_file") # Fallback if cache is missing
...
get_all_sessions() {
    ...
    # Fetch from cache first, then run updates in background if needed
    
    # Try to read cached data immediately
    local tmux_data=$(get_tmux_sessions)
    local tmuxifier_data=$(filter_tmuxifier_sessions) # filter_tmuxifier_sessions calls get_tmuxifier_sessions, which is cached
    local zoxide_data=$(get_zoxide_paths)
    local find_data=$(get_find_paths)

    # Output what we have immediately
    {
        echo "$tmux_data"
        echo "$tmuxifier_data"
        echo "$zoxide_data"
...
# New function to run cache updates in the background
update_caches_in_background() {
    (
        # Detach from the controlling terminal to run truly in background
        nohup bash -c '
            # Re-source options for background process
            source "'"${BASH_SOURCE[0]}"'" # Source self to get load_options and functions
            load_options

            # Force updates by bypassing cache checks (or using 0 expiry for this run)
            # This is a bit hacky, but ensures a fresh cache.
            # A cleaner way would be to pass a flag or have separate update functions.
            # For simplicity, we just call the main functions which will update if expired.
...
toggle_preview() {
    # This function now only sets the option, but the main fzf commands no longer use it
    # We keep the option setting for consistency, but it won't affect the fzf behavior in this version.
    if [ "$(get_tmux_option "@omnimux-preview-enabled" "false")" = "true" ]; then
    ...
    main # Re-run main to reflect the change (though visually, it won't matter now)
}
...
ctrl-p      Toggle preview mode (N/A)
?           Show this help menu
Escape      Exit"
...
main() {
    ...
    # Get current sessions and immediately display them (from cache if fresh enough)
    local all_sessions=$(get_all_sessions)

    # Trigger background updates for the next run
    update_caches_in_background
    ...
    local header_text="ctrl-r:Rename / ctrl-e:Edit / ctrl-t:Terminate / ctrl-d:Delete / ctrl-w:Windows / ctrl-n:New Session / ctrl-p:Preview (N/A) / ?:Help"
